...
const UserModel = models.getModel('user')   // 返回user表.
...
router.get('/shops', function (req, res) {
  ...
  setTimeout(function () {
    ...
    datas.forEach(function (item) {
      data.push({id:item.id, info:item.info});
    });
    res.send({code: 0, data})
  }, 300)
})
...
router.get('/search_shops', function (req, res) {
  ...
  setTimeout(() => {
    const datas = require('../data/data.json');
    datas.forEach((item) => {
      if (item.info.name.indexOf(keyword) != -1) {
        searchShops.push(item);
      }
    })

    if (searchShops) {
      res.send({code: 0, data:searchShops});
    } else {
...
router.get('/shop', function (req, res) {
  ...
  setTimeout(function () {
    const datas = require('../data/data.json');
    const data = datas.find(function (item) {
...
router.get('/found', function (req, res) {
  setTimeout(function () {
    const datas = require('../data/data.json');
    const data = {id:datas[0].id, name:datas[0].info.name, goods:datas[0].goods};
    if (data) {
      res.send({code: 0, data});
    }
  }, 300)
...
// 给用户记录订单
router.post('/recordOrder', function (req, res) {
  const shopName = req.body.name;
  const foods = req.body.foodCart;
  ...
  UserModel.findOne({_id:userid}, _filter, function (err, user) {
    ...
    if (!user) {
      ...
      // 如果有, 记录订单进order
      let obj = {shopName, foods, totalPrice, shopImg, shopId, orderDate};
      // console.log(obj);
      let arr = user.order;
      arr.splice(0, 0, obj);
      //let arr = [];
      UserModel.updateOne({_id:userid}, {order: arr}, function (err, doc) {
        if (err) {
          console.log(err);
